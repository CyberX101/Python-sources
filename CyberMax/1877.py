...
#plugins = str(input('Plugins name : ')
#get_plugin(plugins)
...
#tld = str(input('enter Tld. Domain : ')
#zone(tld)
...
def main():
  ...
  {}===================================================
   ...
   {}Cracked By : CodeBoy1877 | https://1877.to/forums/
  {}===================================================
  ...
  choice = str(input('\nCodeBoy1877@1877 [MENU]: '.format(fy, fr, fre)))
  if (choice == '1'):
    listword = open(str(input('ListWords : '), 'r').read().replace('\n', ' ').strip())
    lis = listword.split(' ')
    pp = Pool(50)
    pp.map(dorker, lis)
  elif (choice == '2'):
    plugins = str(input('enter plugins name : '))
    get_plugin(plugins)
  elif (choice == '3'):
    themes = str(input('enter themes name : '))
    get_theme(themes)
  elif (choice == '4'):
    tld = str(input('enter Tld. Domain : '))
    zone(tld)
  elif (choice == '5'):
    dates = str(input('input date (yyyy-mm-dd): '))
    grab(dates)
  elif (choice == '6'):
    domz = open(str(input("Give Me list to get IP's : "),'r').read().replace('http://', '').replace('https://', '').splitlines())
    Thread = str(input('Thread:~# '))
    th = Pool(int(Thread))
    th.map(domaintoip, domz)
  elif (choice == '7'):
    zizi = str(input('List Ips:~#'))
    with open(zizi) as xy:
      for ipz in xy:
        ipranger(ipz)
  elif (choice == '8'):
    target = open(str(input('List:~# '),'r').read().replace('http://', '').replace('https://', '').splitlines())
    Threadz = str(input('Thread:~# '))
    xx = Pool(int(Threadz))
    xx.map(revip, target)
  elif (choice == '9'):
    tdomain = open(str(input('Input List:~# '),'r').read().replace('http://', '').replace('https://', '').splitlines())
    Threadz = str(input('Thread:~# '))
    xz = Pool(int(Threadz))
